// src/tools/services/salary-receipt-extractor.service.ts
import { Injectable, Logger } from '@nestjs/common';
import {
  ExtractedEmployeeData,
  SalaryReceiptAttendance,
  SalaryReceiptDeduction,
  SalaryReceiptLoan,
  SalaryReceiptPayment,
} from '../interfaces/extracted-employee-data.interface';
...
@Injectable()
export class SalaryReceiptExtractorService {
  private readonly logger = new Logger(SalaryReceiptExtractorService.name);
  ...
  extract(xmlJson: any): ExtractedSalaryReceiptData {
    ...
    this.logger.log(
      `Extraction finished. Total employees extracted: ${employees.length}`,
    );
  ...
  private findEmployeeNodes(root: any): any[] {
    this.logger.debug('Searching employee nodes via direct path...');

    const directSalaryReceiptItems =
    ...
    const directItems = this.toArray(directSalaryReceiptItems).filter((item) =>
      this.hasEmployeeIdentity(item),
    );

    if (directItems.length > 0) {
      this.logger.debug(
        `Found ${directItems.length} employee nodes via direct path.`,
      );
      return directItems;
    }

    this.logger.warn(
      'Direct path for SalaryReceiptItem returned empty, falling back to deep walk search.',
    );

    const result: any[] = [];

    const walk = (node: any, level = 0): void => {
      ...
      if (node.SalaryReceiptItem) {
        ...
        this.logger.debug(
          `Found SalaryReceiptItem collection at level ${level}, count: ${salaryReceiptItems.length}`,
        );
    ...
    this.logger.log(
      `Deep walk search found ${result.length} employee candidate nodes.`,
    );

    return result;
  }

  private hasEmployeeIdentity(node: any): boolean {
    if (!node || typeof node !== 'object') {
      return false;
    }
    ...
    if (!hasId) {
      this.logger.debug(
        `Skipping node (no identity). Node keys: ${
      ...
      this.logger.debug(
        `Candidate employee found. FullName="${fullName}", Code="${personnelCode}"`,
      );
    }
  ...
  private removeDuplicateEmployeeNodes(nodes: any[]): any[] {
    ...
    for (const node of nodes) {
      ...
      if (!seen.has(key)) {
        ...
        this.logger.debug(
          `Duplicate employee node skipped. key="${key}" (FullName="${fullName}", Code="${personnelCode}")`,
        );
      }
    }
  ...
  private mapEmployee(employeeNode: any, root: any): ExtractedEmployeeData {
    ...
    this.logger.debug(
      `Mapping employee: FullName="${fullName}", Code="${personnelCode}"`,
    );
    ...
    // Details (loans, deductions, payments, attendance)
    ...
    this.logger.debug(
      `Employee mapped: FullName="${fullName}", Code="${personnelCode}", NetPayment="${netPayment}"`,
    );
  ...
  private extractLoans(employeeNode: any): SalaryReceiptLoan[] {
    const details4Nodes = this.getDetails4(employeeNode);
    const result: SalaryReceiptLoan[] = [];

    this.logger.debug(
      `Extracting loans. Details4 count: ${details4Nodes.length}`,
    );
  ...
  private extractDeductions(employeeNode: any): SalaryReceiptDeduction[] {
    const details4Nodes = this.getDetails4(employeeNode);
    const result: SalaryReceiptDeduction[] = [];

    this.logger.debug(
      `Extracting deductions. Details4 count: ${details4Nodes.length}`,
    );

    for (const details4 of details4Nodes) {
      const deductionNodes = this.toArray(
        details4?.Column1?.SalaryReceiptDeduction,
      );
  ...
  private extractPayments(employeeNode: any): SalaryReceiptPayment[] {
    const details4Nodes = this.getDetails4(employeeNode);
    const result: SalaryReceiptPayment[] = [];

    this.logger.debug(
      `Extracting payments. Details4 count: ${details4Nodes.length}`,
    );

    for (const details4 of details4Nodes) {
      const paymentNodes = this.toArray(
        details4?.Column2?.SalaryReceiptPayment,
      );
  ...
  private extractAttendance(employeeNode: any): SalaryReceiptAttendance[] {
    const details4Nodes = this.getDetails4(employeeNode);
    const result: SalaryReceiptAttendance[] = [];

    this.logger.debug(
      `Extracting attendance. Details4 count: ${details4Nodes.length}`,
    );

    for (const details4 of details4Nodes) {
      const attendanceNodes = this.toArray(
        details4?.Column3?.SalaryReceiptAttendance,
      );
  ...
  private getDetails4(employeeNode: any): any[] {
    ...
    if (directDetails4Array.length > 0) {
      this.logger.debug(
        `Details4 found via direct path: count=${directDetails4Array.length}`,
      );
      return directDetails4Array;
    }

    this.logger.warn(
      'Details4 direct path empty. Falling back to generic search for "Details4".',
    );
    ...
    this.logger.debug(
      `Details4 found via fallback search: count=${fallbackArray.length}`,
    );
  ...
  private toArray<T = any>(value: T | T[] | null | undefined): T[] {
  ...
  private pickFirst(values: any[]): string | null {
  ...
  private getDirectValue(node: any, key: string): any {
    if (!node || typeof node !== 'object') {
      return undefined;
    }
    ...
    // xml2js: attributes در node.$
    if (node.$ && node.$[key] !== undefined && node.$[key] !== null) {
      return node.$[key];
    }

    // fast-xml-parser: @_<attrName>
    const fastXmlParserAttributeKey = `@_${key}`;
    if (
      node[fastXmlParserAttributeKey] !== undefined &&
  ...
  private getValue(node: any, key: string): any {
    if (!node || typeof node !== 'object') {
      return undefined;
    }
